...
type DataTypeFormatter struct {
	...
	VectorSetDecode func(value any) (map[string]VectorSetElement, error)
	VectorSetEncode func(value map[string]VectorSetElement) (any, error)
...
type DotPip interface {
	...
	VAdd(key Key, element string, vector []float32, options ...VAddOption) (int, error)
	VCard(key Key) (int, error)
	VDim(key Key) (int, error)
	VEmb(key Key, elements ...string) ([][]float32, error)
	VGetAttr(key Key, elements ...string) ([]*string, error)
	VInfo(key Key) (map[string]any, error)
	VIsMember(key Key, elements ...string) ([]bool, error)
	VLinks(key Key, element string) (map[int][]string, error)
	VRandMember(key Key, count int, options ...VRandMemberOption) ([]string, error)
	VRange(key Key, start string, end string, options ...VRangeOption) ([]string, error)
	VRem(key Key, elements ...string) (int, error)
	VSetAttr(key Key, element string, attributes string) (int, error)
	VSim(key Key, reference any, options ...VSimOption) ([]VSimResult, error)
